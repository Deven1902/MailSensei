...
logging.basicConfig(level=logging.INFO, filename='llm.log', filemode='a',
...
class Summarizer():
    def __init__(self):
        self.summarizer = pipeline(
            "summarization", model="sshleifer/distilbart-cnn-12-6", use_fast=True)
        self.detector = pipeline("text-classification",
                                 model="1aurent/distilbert-base-multilingual-cased-finetuned-email-spam", use_fast=True)
        self.tagger = pipeline("text2text-generation",
                               model="fabiochiu/t5-base-tag-generation", use_fast=True)
